class QR:
    ...
    def __init__(self, n, k, message):

        self.format_ = 0
        self.encoded_format = (self.format_ << 10) + self.qr_check_format(self.format_ << 10)
        ...
        self.init_tables(prim=self.prim)

        self.mesecc = self.rs_encode_msg([ord(x) for x in message], n-k)

        self.mesecc_temp = self.mesecc

        self.corrupt_mesecc = self.rs_encode_msg([ord(x) for x in message], n-k)
        ...
        self.mesecc_temp_corrupt = self.corrupt_mesecc

        self.corrected_message, self.corrected_ecc = self.rs_correct_msg(self.corrupt_mesecc, n-k, erase_pos=[0, 1, 2])

        self.__str__()

    # BCH error detection
    def qr_check_format(self, fmt):
    ...
    def qr_decode_format(self, fmt):
        best_fmt = -1
        best_dist = 15
        for test_fmt in range(0,32):
            test_code = (test_fmt<<10) ^ self.qr_check_format(test_fmt<<10)
            test_dist = self.hamming_weight(fmt ^ test_code)
    ...
    # Galois Field multiplication with modular reduction

    def gf_mult_noLUT(self, x, y, prim=0):
        ...
        def cl_div(self, dividend, divisor=None):
        ...
        if prim > 0:
            result = self.cl_div(result, prim)
    ...
    def gf_mul(self, x,y):
        if x==0 or y==0:
            return 0
        return self.gf_exp[self.gf_log[x] + self.gf_log[y]] # should be gf_exp[(gf_log[x]+gf_log[y])%255] if gf_exp wasn't oversized
    ...
    def gf_inverse(self, x):
        return self.gf_exp[255 - self.gf_log[x]] # gf_inverse(x) == gf_div(1, x)

    # Polynomials (multiplies a polynomial by a scalar)
    ...
    def gf_poly_mul(self, p,q):
        ...
        for j in range(0, len(q)):
            for i in range(0, len(p)):
                r[i+j] ^= self.gf_mul(p[i], q[j]) # equivalent to: r[i + j] = gf_add(r[i+j], gf_mul(p[i], q[j]))
                                                            # -- you can see it's your usual polynomial multiplication
        return r

    # Horner's method, evaluate a polynomial at a particular value of x, producing a scalar result. 
    ...
    def rs_generator_poly(self, nsym):
        ...
        for i in range(0, nsym):
            g = self.gf_poly_mul(g, [1, self.gf_pow(2, i)])
        return g
    ...
    def rs_find_errata_locator(self, e_pos):
        ...
        for i in e_pos:
            e_loc = self.gf_poly_mul( e_loc, self.gf_poly_add([1], [self.gf_pow(2, i), 0]) )
        return e_loc
    ...
    # Forney algorithm
    def rs_correct_errata(self, msg_in, synd, err_pos): # err_pos is a list of the positions of the errors/erasures/errata
        ...
        for i, Xi in enumerate(X):
            ...
            y = self.gf_mul(self.gf_pow(Xi, 1), y)
            
            # Check: err_loc_prime (the divisor) should not be zero.
            if err_loc_prime == 0:
                raise self.ReedSolomonError("Could not find error magnitude")    # Could not find error magnitude
    ...
    def rs_find_error_locator(self, synd, nsym, erase_loc=None, erase_count=0):
        ...
        if (errs-erase_count) * 2 + erase_count > nsym:
            raise self.ReedSolomonError("Too many errors to correct")    # too many errors to correct
    ...
    def rs_find_errors(self, err_loc, nmess): # nmess is len(msg_in)
        ...
        for i in range(nmess): # normally we should try all 2^8 possible values, but here we optimize to just check the interesting symbols
            if self.gf_poly_eval(err_loc, self.gf_pow(2, i)) == 0: # It's a 0? Bingo, it's a root of the error locator polynomial,
                                                                                    # in other terms this is the location of an error
                err_pos.append(nmess - 1 - i)
        # Sanity check: the number of errors/errata positions found should be exactly the same as the length of the errata locator polynomial
        if len(err_pos) != errs:
            # couldn't find error locations
            raise self.ReedSolomonError("Too many (or few) errors found by Chien Search for the errata locator polynomial!")
        return err_pos
    ...
    def rs_forney_syndromes(self, synd, pos, nmess):
        ...
        for i in range(0, len(pos)):
            x = self.gf_pow(2, erase_pos_reversed[i])
    ...
    def rs_correct_msg(self, msg_in, nsym, erase_pos=None):
        ...
        if len(erase_pos) > nsym: raise self.ReedSolomonError("Too many erasures to correct")
        ...
        if err_pos is None:
            raise self.ReedSolomonError("Could not locate error")    # error location failed
        ...
        msg_out = self.rs_correct_errata(msg_out, synd, (erase_pos + err_pos)) # note that we here use the original syndrome, not the forney syndrome
                                                                                                                                    # (because we will correct both errors and erasures, so we need the full syndrome)
        # check if the final message is fully repaired
        synd = self.rs_calc_syndromes(msg_out, nsym)
        if max(synd) > 0:
            raise self.ReedSolomonError("Could not correct message")     # message could not be repaired
        # return the successfully decoded message
        return msg_out[:-nsym], msg_out[-nsym:] # also return the corrected ecc block so that the user can check()

    def __str__(self) -> str:
        return print(
                    "Original: " + str(self.mesecc), 
                    "\nCorrupted: " + str(self.corrupt_mesecc),
                    "\nRepaired: " + str(self.corrected_message + self.corrected_ecc), 
                    "\n" + ''.join([chr(x) for x in self.corrected_message]),
